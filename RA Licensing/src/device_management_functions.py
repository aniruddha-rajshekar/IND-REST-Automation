...
###########################################################################################################################################
@handle_exception_wrapper
def retrieve_license_info_by_serial(ind_info, serial_number, license_info_parameter, return_json):
    assert (type(ind_info) == dict and sorted(list(ind_info.keys())) == sorted(ind_info_keys_list)), \
        'Credentials dictionary incorrect. Verify  correct credentials/ check ind_info_keys_list'
    assert type(serial_number) == str, 'Invalid IP address'
    assert type(license_info_parameter) == str, 'License information not a string'

    url = 'https://' + ind_info['ip'] + ':8443/api/v1/licensing/files'
    [license_files_get_response, license_files_get_json] = request_get(url, (ind_info['username'], ind_info['password']))
    assert (license_files_get_response.status_code == 200 and license_files_get_json['status'] == 200), \
        'GET License files response incorrect'

    if return_json:
        license_file_info = [license_files_get_json['records'][i]
                             for i in range(0, license_files_get_json['recordCount'])
                             if serial_number == license_files_get_json['records'][i]['serialNumber']]
    else:
        license_file_info = [license_files_get_json['records'][i][license_info_parameter]
                             for i in range(0, license_files_get_json['recordCount'])
                             if serial_number == license_files_get_json['records'][i]['serialNumber']]

    if len(license_file_info) == 0:
        log.info('\tFailed to find License file: ' + str(serial_number))
        return 'null'

    return license_file_info[0]
...
def change_state(ind_info, devices_id_list, to_state):
    ...
    try:
        ...
        return str(devices_put_json['record']['message'])
