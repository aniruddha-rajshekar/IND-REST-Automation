...
###########################################################################################################################################
###########################################################################################################################################
def run_all_test_cases(script_file, yaml_file):
    global test_path
    log.info('\tCalling run_all_test_cases job function for ' + str(yaml_file))

    try:
        ind_status_message_dict = read_dict_data()
        log.info(pformat(ind_status_message_dict))
        testbed = loader.load(test_path + '/' + yaml_file)
        ...
        for key1 in sorted(s8000.custom):
            ...
            run(testscript=test_path + '/' + script_file, testcasename=h_method, testcaseid=test_case_id,
        ...
        log.info('\t' + str(yaml_file) + ' Job Failed: ' + str(msg))


###########################################################################################################################################
###########################################################################################################################################
def run_my_scripts(csv_file_name):
    global test_path

    test_path = (os.path.dirname(os.path.abspath(__file__)))
    log.info('\tStart running all Device Management scripts')
    try:
        i_file = open(test_path + '/' + csv_file_name, 'r')
        log.info('\tYaml Files ==> ' + test_path + '/' + csv_file_name)
        ...
        for row in reader:
            ...
            if row_num == 0:
                ...
                script_f = row[0]
                yaml_f = row[1]
                log.info('\t' + str(script_f))
                log.info('\t' + str(yaml_f))
                run_all_test_cases(script_f, yaml_f)
            row_num += 1
        i_file.close()
