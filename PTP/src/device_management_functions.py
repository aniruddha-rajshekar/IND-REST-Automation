...
###########################################################################################################################################
@handle_exception_wrapper
def retrieve_supported_device_port_info_by_ip(ind_info, port_name, device_id, device_info_parameter, return_json):
    ...
    assert type(port_name) == str, 'Invalid Port Name'
    ...
    url = 'https://' + ind_info['ip'] + ':8443/api/v1/supported-devices/' + str(device_id) + '/ports-details?direction=ASC'
    [ports_get_response, ports_get_json] = request_get(url, (ind_info['username'], ind_info['password']))
    assert (ports_get_response.status_code == 200 and ports_get_json['status'] == 200), \
        'GET Ports response incorrect'

    if return_json:
        port_info = [ports_get_json['records'][i]
                                 for i in range(0, ports_get_json['recordCount'])
                                 if port_name == ports_get_json['records'][i]['name']]
    else:
        port_info = [ports_get_json['records'][i][device_info_parameter]
                                 for i in range(0, ports_get_json['recordCount'])
                                 if port_name == ports_get_json['records'][i]['name']]

    if len(port_info) == 0:
        log.info('\tFailed to find Port: ' + str(port_name) + ' on Device: ' + str(device_id))
        return 'null'
...
###########################################################################################################################################
@handle_exception_wrapper
def retrieve_ptp_id_from_db(self, device_id):
    assert type(device_id) == int, 'Invalid Device ID'
    ...
    if 'cip_slot' in self.param_in:
        ptp_id = [ptp_instances_get_json['records'][i]['id'] for i in range(0, ptp_instances_get_json['recordCount'])
                  if str(ptp_instances_get_json['records'][i]['cipSlot']) == str(self.param_in['cip_slot'])][0]
    else:
        ptp_id = [ptp_instances_get_json['records'][i]['id'] for i in range(0, ptp_instances_get_json['recordCount'])][0]
...
###########################################################################################################################################
@handle_exception_wrapper
def get_ptp_id_for_offset_test(self, device_id):
    assert type(device_id) == int, 'Invalid Device ID'

    if str(self.expected_in[0]) == '404' or str(self.expected_in[0]) == '400':
        if self.param_in['licensed'] == 'False' or self.param_in['is_ptp'] == 'False':
            ptp_id = device_id

        elif self.param_in['is_gm'] == 'True' or self.param_in['is_ptp'] == 'True':
            if 'ptp_device_id' in self.param_in:
                ptp_id = self.param_in['ptp_device_id']
            else:
                ptp_id = retrieve_ptp_id_from_db(self, device_id)
    else:
        ptp_id = retrieve_ptp_id_from_db(self, device_id)
...
def change_state(ind_info, devices_id_list, to_state):
    ...
    try:
        ...
        [devices_post_response, devices_post_json] = request_post(url, (ind_info['username'], ind_info['password']),
                                                                  {'ids': devices_id_list, 'newDeviceAdminStateStr': str(to_state)})
        assert (devices_post_response.status_code == 200 and devices_post_json['status'] == 200), \
            'POST Devices State Change response incorrect'

        log.info('\tState change Task ID: ' + str(devices_post_json['record']['taskId']))
        wait_for_task_completion(ind_info, str(devices_post_json['record']['taskId']))
...
###########################################################################################################################################
@handle_exception_wrapper
def perform_topology_by_group(ind_info, group_id):
    assert (type(ind_info) == dict and sorted(list(ind_info.keys())) == sorted(ind_info_keys_list)), \
        'Credentials dictionary incorrect. Verify  correct credentials/ check ind_info_keys_list'
    assert type(group_id) == int, 'Group ID is not an int'

    log.info('\tPerforming Topology for group ' + str(group_id) + '...')
    url = 'https://' + ind_info['ip'] + ':8443/api/v1/topology/discoveries/tasks'
    [topology_post_response, topology_post_json] = request_post(url, (ind_info['username'], ind_info['password']), group_id)
    log.info(pformat(topology_post_json))
    assert (topology_post_response.status_code == 200 and topology_post_json['status'] == 200), \
        'POST Topology response incorrect'
    log.info('\tTopology Task ID: ' + str(topology_post_json['record']['id']))
