...
from dlr_test_functions import *
...
def dlr(self):
    log.info('\tCalling DLR Method: ' + str(self.h_method))
    log.info('\tPrinting DLR param_in: ' + str(self.param_in))
    param_in_dict = self.param_in
    assert type(param_in_dict) == dict, 'Parameters In is not a dictionary'

    #######################################################################################################################################
    if self.h_method == 'GETdlrinstancesbygroup':
        get_dlr_instances_by_group(self, param_in_dict)

    #######################################################################################################################################
    elif self.h_method == 'GETdlrtopologybygroup':
        get_dlr_topology_by_group(self, param_in_dict)

    #######################################################################################################################################
    elif self.h_method == 'GETdlrdevicedetailsbyid':
        get_dlr_device_details_by_id(self, param_in_dict)

    #######################################################################################################################################
    elif self.h_method == 'GETdlrnodesummarybyid':
        get_dlr_node_summary_by_id(self, param_in_dict)

    #######################################################################################################################################
    else:
        self.failed('\tNo matching hMethod for DLR')
...
class CommonSetup(aetest.CommonSetup):
    """ Common Setup for DLR Automation Testing """

    @aetest.subsection
    def common_setup_subsection(self, testcaseid):
        log.info('\tDLR Automation Common Setup')
        DLRAutomation.id = testcaseid
...
class DLRAutomation(aetest.Testcase):
    """ Test Case Class for DLR Automation """

    @aetest.setup
    def dlr_setup(self):
        """ Test Case Setup for DLR Automation """
        log.info('\tDLR Automation Testcase Setup')

    @aetest.test
    def run_dlr_automation_test_case(self):
        ...
        try:
            ...
            if self.func_name == 'dlr':
                log.info('\tCalling DLR')
                dlr(self)
                ...
                self.failed('\tNo matching func_name for DLR Tests')

        except Exception as msg:
            self.failed('\tRun DLR Test Case Failed: ' + str(msg))

    @aetest.cleanup
    def dlr_automation_cleanup(self):
        """ Test Case Cleanup for DLR Automation """
        log.info('\tDLR Automation Test Case Cleanup')
...
class CommonCleanup(aetest.CommonCleanup):
    """ Common Cleanup for DLR Automation Testing """

    @aetest.subsection
    def common_cleanup_subsection(self, testcaseid):
        log.info('\tDLR Automation Testing Complete')
